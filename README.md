...
Author-initials resolution and author–task assignment
    ↓
Deterministic CRediT classification + Centroid-based semantic classification
    ↓
...
>
> **Semantic-classification naming note.** The historical notebook name `credit_assignment_rules_and_fewshot (1).ipynb`, together with legacy names such as `few_shot_predictions.csv` and `credit_send_to_few_shot`, uses the earlier label *few-shot*. In the manuscript and throughout the current methodological description, this stage is referred to as **Centroid-based semantic classification**. These names refer to the same semantic fallback stage: unresolved atomic actions are embedded, compared with frequency-weighted CRediT role centroids, and accepted using role-specific similarity and margin thresholds. The historical notebook, file, and column names are retained only for backward compatibility.
...
| 3 | `credit_assignment_rules_and_fewshot (1).ipynb` | `03_assign_credit_roles.ipynb` | Map free-text contribution tasks to the 14 CRediT roles using deterministic rules followed, only for unresolved atomic actions, by the **Centroid-based semantic classification** layer. |
...
| Atomic-action level | One action internally extracted from an original task | Multi-action classification and Centroid-based semantic classification. |
...
7. Unresolved status and controlled transfer to Centroid-based semantic classification.
...
### Centroid-based semantic classification

Only unresolved atomic actions are processed by this semantic layer.
...
The Centroid-based semantic classification layer does not overwrite deterministic assignments.

This notebook provides the core H-Contrib normalization layer. It combines transparent deterministic evidence with a restricted centroid-based semantic fallback and retains enough provenance to audit every final assignment.
...
| `credit_assignment_method` | Rule, explicit label, nominal phrase, dependency rule, centroid-based semantic classification (historically stored as `few_shot` in some outputs), or unresolved. |
| `credit_send_to_few_shot` | Historical column name indicating whether deterministic processing left the action unresolved and therefore sent it to the Centroid-based semantic classification stage. |
...
6. Inspect unresolved tasks and `few_shot_predictions.csv` (historical filename for the Centroid-based semantic classification output).
...
| `few_shot_predictions.csv` | Historical filename for the Centroid-based semantic classification output; contains similarity, threshold, score margin, and nearest example. |
...
- only unresolved actions reach the Centroid-based semantic classifier;
...
- Centroid-based semantic classification results depend on the quality and diversity of the verified examples.
